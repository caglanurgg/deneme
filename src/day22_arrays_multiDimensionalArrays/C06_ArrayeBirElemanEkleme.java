...
public class C06_ArrayeBirElemanEkleme {
    public static void main(String[] args) {
        ...
        arr = yeniArr;
        // yeni olusturdugumuz ve eskisinden bir uzun olan sonuna da 10 eklenmis olan array'in degerini
        // eski array'e atadik
    ...
    public static int [] arrayeBirElemanEkle (int[] eskiArr,int eklenecekSayi){ // bana yeni halini dondurmesi lazim

        int [] yeniArr = new int[eskiArr.length+1];

        for (int i = 0; i < eskiArr.length ; i++) {

            yeniArr[i] =  eskiArr [i];  // 3- eklenecek elementi, yeniArr'in son index'ine atayalim
        }
        yeniArr [yeniArr.length-1] = eklenecekSayi ;

        eskiArr =yeniArr ; // 4- eski arr'ye yeni degeri atayalim
